...
function computeAll(entries) {
  ...
  const marcas = {};        // marca -> points  (TODAS las marcas, no solo Bavaria)
  ...
  entries.forEach(e => {
    ...
    const marcaLabel = marcaGroupLabel(e.marca);
    marcas[marcaLabel] = (marcas[marcaLabel] || 0) + pts;
...
function countsFor(entries) {
  ...
  // "Shortlists" is the total number nominated in the first place — it never
  // shrinks as ideas get upgraded to a medal, so show the full count here.
  counts.Finalista = entries.length;
  return counts;
}
...
function renderMedalStrip(prefix, counts) {
  ...
  $(`#${prefix}GranEffie`).textContent = counts['Gran Effie'] || 0;
}
...
function renderRace(container, dataObj, opts) {
  ...
  entries.forEach(([name, val]) => {
    const isUs = opts.highlightSet ? opts.highlightSet.has(name) : (opts.highlight && opts.highlight === name);
    const displayName = opts.displayLabel ? opts.displayLabel(name) : name;
...
function findMatches(query) {
  ...
  if (entries.length) {
    const marcaLabel = marcaGroupLabel(entries[0].marca);
    return { entries, label: marcaLabel === 'Bavaria' ? 'Bavaria (corporativa)' : marcaLabel, type: 'Marca' };
  }
...
function renderHome() {
  ...
  const bavariaEntries = dataset.filter(e => isBavariaBrand(e.marca));
  const otrosEntries = dataset.filter(e => !isBavariaBrand(e.marca));
  renderMedalStrip('cntBavaria', countsFor(bavariaEntries));
  renderMedalStrip('cntOtros', countsFor(otrosEntries));

  const bavariaMarcaLabels = new Set(bavariaEntries.map(e => marcaGroupLabel(e.marca)));
  ...
  renderRace($('#raceMarca'), totals.marcas, {
    highlightSet: bavariaMarcaLabels,
    limit: 8,
    displayLabel: (name) => name === 'Bavaria' ? 'Bavaria (corporativa)' : name,
...
function renderIdeaRows(container, entries, opts) {
  opts = opts || {};
  ...
  container.innerHTML = '';
  if (!sorted.length) {
    container.innerHTML = `<p class="race-empty">${opts.emptyText || 'Nada por aquí todavía.'}</p>`;
    return;
  }
  sorted.forEach(e => {
    ...
    const marcaPrefix = opts.showMarca ? `<b>${marcaGroupLabel(e.marca)}</b> · ` : '';
    body.innerHTML = `
      <div class="campana">${e.campana}</div>
      <div class="meta">${marcaPrefix}<b>${e.categoria}</b> · ${agencia}</div>`;
...
function showEntity(query) {
  ...
  renderIdeaRows($('#marcaIdeasList'), entries, { showMarca: type !== 'Marca' });

  showView('view-marca');
}

const METAL_DISPLAY_NAME = { Finalista: 'Shortlists', Bronce: 'Bronce', Plata: 'Plata', Oro: 'Oro', 'Gran Effie': 'Gran Effie' };

function showMetal(metalName, scope) {
  const dataset = buildDataset();
  const inScope = scope === 'bavaria'
    ? dataset.filter(e => isBavariaBrand(e.marca))
    : dataset.filter(e => !isBavariaBrand(e.marca));
  const entries = metalName === 'Finalista'
    ? inScope
    : inScope.filter(e => e.metal === metalName);
  const label = METAL_DISPLAY_NAME[metalName] || metalName;
  const scopeLabel = scope === 'bavaria' ? 'Bavaria' : 'Resto de la industria';

  $('#marcaName').textContent = `${label} · ${scopeLabel}`;
  renderMedalStrip('mCnt', countsFor(inScope));
  $('#marcaGrandEffieBadge').hidden = true;

  renderIdeaRows($('#marcaIdeasList'), entries, {
    showMarca: true,
    emptyText: `${scopeLabel} todavía no tiene nada en ${label.toLowerCase()}.`
  });
...
/* ---------- Real submission to Google Forms via a hidden form + iframe ----------
   More reliable than a silent fetch(): this mimics an actual person filling
   and submitting the form, which Google Forms always accepts, instead of a
   background request that can get silently blocked. */
function submitToGoogleForm(actionUrl, fields) {
  return new Promise((resolve) => {
    ...
    form.submit();
    setTimeout(() => { form.remove(); resolve(); }, 900);
  });
}
...
async function submitForm() {
  ...
  if (envio && envio.activo && envio.formActionUrl && !envio.formActionUrl.startsWith('PEGA_AQUI')) {
    try {
      const fields = {
        [envio.entryIds.marca]: marca,
        [envio.entryIds.campana]: campana,
        [envio.entryIds.categoria]: categoria,
        [envio.entryIds.agenciaLider]: agenciaLider,
        [envio.entryIds.agenciaContribuyente]: agenciaContrib,
        [envio.entryIds.metal]: metal
      };
      await submitToGoogleForm(envio.formActionUrl, fields);
...
async function boot() {
  ...
  $$('.medal-strip.clickable .medal-box').forEach(box => {
    box.addEventListener('click', () => showMetal(box.dataset.metal, box.dataset.scope));
  });
  ...
  $('#clearLocalBtn').addEventListener('click', () => {
    const n = LOCAL_ROWS.length;
    if (!n) { alert('No tienes pruebas guardadas en este teléfono.'); return; }
    if (!confirm(`Esto borra ${n} entrada(s) guardadas solo en este teléfono (no toca lo que ya esté en la hoja compartida). ¿Seguro?`)) return;
    LOCAL_ROWS = [];
