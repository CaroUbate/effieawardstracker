...
function advertiserBucket(marca) {
  return isBavariaBrand(marca) ? 'Bavaria' : canonicalMarca(marca);
}
...
function buildDataset() {
  ...
  const incoming = [...SYNCED_ROWS, ...LOCAL_ROWS];
...
function computeAll(entries) {
  ...
  entries.forEach(e => {
    ...
    const bucket = advertiserBucket(e.marca);
    anunciantes[bucket] = (anunciantes[bucket] || 0) + pts;

    if (isBavariaBrand(e.marca)) {
...
function renderHome() {
  ...
  renderRace($('#raceMarca'), totals.marcas, {
    onClick: (marca) => showMarca(marca)
  });

  // datalist for search
  const marcasSet = new Set(dataset.map(e => canonicalMarca(e.marca)).filter(Boolean));
  const dl = $('#marcasList');
  dl.innerHTML = '';
...
function showMarca(marca) {
  const dataset = buildDataset();
  const entries = dataset.filter(e => norm(e.marca) === norm(marca));
  if (!entries.length) return;

  $('#marcaName').textContent = canonicalMarca(marca);
...
function initFormOptions() {
  ...
  $('#ff_marca').addEventListener('input', updateCampanaOptions);
}
...
async function boot() {
  ...
  $('#searchInput').addEventListener('change', (e) => {
    ...
    if (val) showMarca(val);
  });
  $('#searchInput').addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && e.target.value.trim()) showMarca(e.target.value.trim());
  });
